...
for i in $(seq 0 ${l})
do
    TpmC=$TpmC"\"${RESULT_PATH}/tmp-"${CC[$i]}"\" using 2:xtic(1) with histogram title  \"${CC[$i]}\""
    Rollback=$Rollback"\"${RESULT_PATH}/tmp-"${CC[$i]}"\" using (3*100):xtic(1) with histogram title \"${CC[$i]}\""
    Distributed=$Distributed"\"${RESULT_PATH}/tmp-"${CC[$i]}"\" using (4*100):xtic(1) with histogram title \"${CC[$i]}\""
    if [ $i != ${l} ]
    then
        TpmC=$TpmC","
        Rollback=$Rollback","
