...
void RouteTable::set_route_node_watermark(uint64_t partition_id, uint64_t node_id,
                                    uint64_t watermark, uint64_t thd_id) {
    uint64_t id0 = get_primary_node_id(partition_id);
    uint64_t id1 = get_follower1_node_id(partition_id);
    uint64_t id2 = get_follower2_node_id(partition_id);
    if (node_id == id0) {
        table[partition_id].primary.watermark = watermark;
    } else if (node_id == id1) {
        table[partition_id].secondary_1.watermark = watermark;
    } else if (node_id == id2) {
        table[partition_id].secondary_2.watermark = watermark;
    } else {
        assert(false);
    }
}
...
void RouteTable::set_remote_route_node_watermark(yield_func_t &yield, uint64_t partition_id, uint64_t node_id,
    ...
    uint64_t id0 = tmp_node->primary.node_id;
    uint64_t id1 = tmp_node->secondary_1.node_id;
    uint64_t id2 = tmp_node->secondary_2.node_id;
    uint64_t orig_watermark;
    uint64_t index1;
    if (node_id == id0) {
        index1 = 0;
        orig_watermark = tmp_node->primary.watermark;
    } else if (node_id == id1) {
        index1 = 1;
        orig_watermark = tmp_node->secondary_1.watermark;
    } else if (node_id == id2) {
        index1 = 2;
        orig_watermark = tmp_node->secondary_2.watermark;
    } else {
        assert(false);
    }
    ...
    while (orig_watermark < watermark) {
        rc = cas_remote_route_node_watermark(yield, node_id, partition_id, index1, orig_watermark, watermark, &remote_value, thd_id, cor_id);
...
route_node_ts RouteTable::get_primary(uint64_t partition_id) {
    return table[partition_id].primary;
}
