...
class KeyXidEnt : public KeyRangeEnt {
 public:
  ...
  KeyXidEnt(uint64_t Sk, uint64_t Ek) : KeyRangeEnt(Sk, Ek), writeTxn(0), isDelete(false) {};
  ...
  KeyXidEnt(const KeyRangeEnt& ent, const std::vector<uint64_t>& readTxn_, uint64_t writeTxn_)
      : readTxn(readTxn_), writeTxn(writeTxn_) {
    StartKey = ent.StartKey;
    EndKey = ent.EndKey;
  }
...
class KeyXidCache {
 public:
  ...
  std::vector<uint64_t> addWriteXidWithMutex(uint64_t StartKey, uint64_t EndKey, uint64_t txn,
                                             int32_t isolation, bool is_delete);
