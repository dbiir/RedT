...
void TxnManager::cleanup_row(RC rc, uint64_t rid,vector<vector<uint64_t>>& remote_access) {
...
#if CC_ALG != SILO && CC_ALG != RDMA_NO_WAIT && CC_ALG != RDMA_NO_WAIT2 && CC_ALG != RDMA_WAIT_DIE2
...
//TODO-relase lock
	...
	for (int rid = row_cnt - 1; rid >= 0; rid --) {
		cleanup_row(rc,rid,remote_access);  //return abort write row
	}
    ...
    for(int i=0;i<g_node_cnt;i++){ //for the same node, batch unlock remote
        if(remote_access[i].size() > 0){
            batch_unlock_remote(i, Abort, this, remote_access);
        }
    }
#if USE_OR == true
    for(int i=0;i<g_node_cnt;i++){ //poll result
        if(remote_access[i].size() > 0){
            auto dbres1 = rc_qp[i][get_thd_id()]->wait_one_comp();
...
#if USE_DBPA == true
row_t * TxnManager::cas_and_read_remote(uint64_t& try_lock, uint64_t target_server, uint64_t cas_offset, uint64_t read_offset, uint64_t compare, uint64_t swap){
...
#if CC_ALG == RDMA_NO_WAIT || CC_ALG == RDMA_NO_WAIT2 || CC_ALG == RDMA_WAIT_DIE2 || CC_ALG == RDMA_SILO || CC_ALG == RDMA_TS1 || CC_ALG == RDMA_MVCC 
...
#if CC_ALG == RDMA_NO_WAIT || CC_ALG == RDMA_NO_WAIT2 || CC_ALG == RDMA_WAIT_DIE2 || CC_ALG == RDMA_SILO
...
row_t * TxnManager::read_remote_row(yield_func_t &yield, uint64_t target_server,uint64_t remote_offset, uint64_t cor_id){
	...
	uint64_t waitcomp_time = get_sys_clock();
	INC_STATS(get_thd_id(), worker_idle_time, waitcomp_time - h_thd->last_yield_time);
...
RdmaTimeTableNode * TxnManager::read_remote_timetable(yield_func_t &yield, uint64_t target_server,uint64_t remote_offset,uint64_t cor_id){
	uint64_t operate_size = sizeof(RdmaTimeTableNode);
