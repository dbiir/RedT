...
void YCSBTxnManager::get_num_msgs_statistics() {
	...
	for(uint64_t i=0;i<g_center_cnt;i++){
		if(i == g_center_id){
			num_msgs_rw += m*w_pry_num[i];
			num_msgs_commit += m*w_pry_num[i];
		}else{
			num_msgs_rw += (m+1)*w_pry_num[i] + r_pry_num[i];
			num_msgs_commit += (m+1)*w_pry_num[i] + r_pry_num[i];
		}
	}
	num_msgs_rw += M;
#else
	...
	for(uint64_t i=0;i<g_center_cnt;i++){
		if(i == g_center_id){
			num_msgs_prep += m*w_pry_num[i];
			num_msgs_commit += m*w_pry_num[i];
		}else{
			num_msgs_rw += w_pry_num[i] + r_pry_num[i];
			num_msgs_prep += (m+1)*w_pry_num[i];
			num_msgs_commit += (m+1)*w_pry_num[i] + r_pry_num[i];
		}
	}
	num_msgs_prep += M;
