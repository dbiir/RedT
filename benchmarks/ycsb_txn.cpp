...
RC YCSBTxnManager::run_txn(yield_func_t &yield, uint64_t cor_id) {
  ...
  if (IS_LOCAL(txn->txn_id) && state == YCSB_0 && next_record_id == 0) {
    ...
    if (is_readonly() && read_only_optimization() != Abort) {
...
RC YCSBTxnManager::ycsb_read_remote_index(yield_func_t &yield, ycsb_request *req, itemid_t *&item,
                                          uint64_t cor_id) {
  uint64_t part_id = _wl->key_to_part(req->key);
  ...
  for (int j = 0; j < req->replica_cnt; j++) {
    uint64_t loc1 = get_node_id_new(j, part_id);
    if (GET_CENTER_ID(loc1) == g_center_id) {
      loc = loc1;
      break;
    }
  }
...
RC YCSBTxnManager::send_remote_one_side_request(yield_func_t &yield, ycsb_request *req,
                                                row_t *&row_local, uint64_t cor_id) {
  ...
  uint64_t loc = -1;
#if REPLICA_COUNT != 0
  req->replica_cnt = get_part_repl_cnt(part_id);
  for (int j = 0; j < req->replica_cnt; j++) {
    uint64_t loc1 = get_node_id_new(j, part_id);
    if (GET_CENTER_ID(loc1) == g_center_id) {
      loc = loc1;
...
RC YCSBTxnManager::send_remote_subtxn() {
  ...
  // printf("txn %lu, req cnt %ld\n", get_txn_id(),ycsb_query->requests.size());
  for (int i = 0; i < ycsb_query->requests.size(); i++) {
    ycsb_request *req = ycsb_query->requests[i];
    uint64_t part_id = _wl->key_to_part(req->key);
    if (!is_count[part_id]) {
      access_count[part_id]++;
      is_count[part_id] = true;
    }
    vector<uint64_t> node_id_list;
    vector<uint64_t> part_id_list;
    uint64_t loc = -1;
#if USE_REPLICA
#if REPLICA_COUNT != 0
    if (REPLICA_CC || req->acctype == WR) {
      ...
      for (int j = 0; j < req->replica_cnt; j++) {
        ...
        node_id_list.push_back(loc);
        part_id_list.push_back(part_id);
        req->replica_node[j].stored_node = loc;
      }
    } else if (req->acctype == RD) {
      // 目前只在第一个副本上读
      // !之后需要结合延迟选最近的
      req->replica_cnt = 1;
      loc = get_node_id_new(0, part_id);
      if (loc == -1) return Abort;
      node_id_list.push_back(loc);
      part_id_list.push_back(part_id);
      req->replica_node[0].stored_node = loc;
    } else
      assert(false);
#else
    if (REPLICA_CC || req->acctype == WR) {
    ...
    req->primary.stored_node = loc;
#endif
    for (int j = 0; j < node_id_list.size(); j++) {
      uint64_t center_id = GET_CENTER_ID(node_id_list[j]);
      remote_center[center_id].push_back(i);
      ycsb_query->centers_touched.add_unique(center_id);
      ycsb_query->partitions_touched.add_unique(GET_PART_ID(0, part_id_list[j]));
      // center_master is set as the first toughed primary, if not exist, use the first toughed
      // backup.
      auto ret = center_master.insert(pair<uint64_t, uint64_t>(center_id, node_id_list[j]));
      if (ret.second == false) {
        if (!is_primary[center_id] && j == 0) {
          center_master[center_id] = node_id_list[j];  // change center_master
          is_primary[center_id] = true;
        }
      } else {
        is_primary[center_id] = (j == 0 ? true : false);
      }
      // printf("txn %lu, req %ld center %ld node id %ld  center master size %ld\n", get_txn_id(),i,center_id,center_master[center_id],center_master.size());
    }
  }
  ...
  for (auto iter = center_master.begin(); iter != center_master.end(); iter++) {
    ...
    // printf("txn %lu, center_id %ld\n",get_txn_id(),center_id,executore_id);
    for (int i = 0; i < ycsb_query->requests.size(); i++) {
      ycsb_request *req = ycsb_query->requests[i];
#if REPLICA_COUNT != 0
      for (int j = 0; j < req->replica_cnt; j++) {
        if (GET_CENTER_ID(req->replica_node[j].stored_node) == center_id) {
          req->replica_node[j].execute_node = executore_id;
          // printf("txn %lu, node %ld needs to handle req %d's %ldrd replica\n", get_txn_id(),executore_id, i, j);
        }
      }
#else
  ...
  // printf("txn %lu, needs send inter-txn to %lu nodes\n", get_txn_id(), rsp_cnt);
  ...
  for (int i = 0; i < g_center_cnt; i++) {
    if (remote_center[i].size() > 0 && i != g_center_id) {  // send message to all masters
      ...
      // printf("txn %lu, send inter-txn to %lu\n", get_txn_id(), center_master[i]);
    }
  }
...
RC YCSBTxnManager::read_only_optimization() {
  ...
  for (int i = 0; i < ycsb_query->requests.size(); i++) {
...
#if USE_REPLICA
#if REPLICA_COUNT != 0
    assert(req->acctype == RD);
    req->replica_cnt = 1;
    uint64_t replica_cnt = get_part_repl_cnt(part_id);

    bool has_same_center = false;
    for (int j = 0; j < replica_cnt; j++) {
      loc = get_node_id_new(j, part_id);
      if (loc == -1) return Abort;
      uint64_t center_id = GET_CENTER_ID(loc);
      if (center_id == GET_CENTER_ID(g_node_id)) {
        has_same_center = true;
        node_id.push_back(loc);
        req->replica_node->stored_node = loc;
    ...
    if (!has_same_center) return Abort;
#else
    if (REPLICA_CC || req->acctype == WR) {
      loc = get_primary_node_id(part_id);
      if (loc == -1) return Abort;
  ...
  for (auto iter = center_master.begin(); iter != center_master.end(); iter++) {
    ...
    for (int i = 0; i < ycsb_query->requests.size(); i++) {
      ...
      for (int j = 0; j < req->replica_cnt; j++) {
        if (GET_CENTER_ID(req->replica_node[j].stored_node) == center_id) {
          req->replica_node[j].execute_node = executore_id;
          DEBUG_T("txn %lu, node %ld needs to handle req %d's %ldrd replica\n", get_txn_id(),
                  executore_id, i, j);
        }
      }
#else
...
RC YCSBTxnManager::send_remote_request() {
  ...
  tport_man.rdma_thd_send_msg(get_thd_id(), this->center_master[dest_center_id],
  ...
  msg_queue.enqueue(get_thd_id(), Message::create_message(this, RQRY),
                    this->center_master[dest_center_id]);
#endif
#else
#if USE_RDMA == CHANGE_MSG_QUEUE
...
void YCSBTxnManager::copy_remote_requests(YCSBQueryMessage *msg) {
  ...
  while (next_record_id < ycsb_query->requests.size() && !is_local_request(next_record_id) &&
         GET_CENTER_ID(GET_NODE_ID(_wl->key_to_part(ycsb_query->requests[next_record_id]->key))) ==
             dest_center_id) {
#else
  while (next_record_id < ycsb_query->requests.size() && !is_local_request(next_record_id) &&
         GET_NODE_ID(_wl->key_to_part(ycsb_query->requests[next_record_id]->key)) == dest_node_id) {
...
// if (node_id != req->primary.stored_node) return Abort;
  ...
  bool is_local =
      (req->primary.stored_node == g_node_id && req->primary.execute_node == g_node_id) ||
      (req->second1.stored_node == g_node_id && req->second1.execute_node == g_node_id) ||
      (req->second2.stored_node == g_node_id && req->second2.execute_node == g_node_id);
#endif
#else
  bool is_center = req->primary.execute_node == g_node_id;
  ...
  switch (state) {
    case YCSB_0:
      ...
      break;
    case YCSB_1:
      // read local row,for message queue by TCP/IP,write set has actually been written in this
      // point, but for rdma, it was written in local, the remote data will actually be written when
      // COMMIT
      if (is_center)
...
RC YCSBTxnManager::run_ycsb_0(yield_func_t &yield, ycsb_request *req, row_t *&row_local,
                              uint64_t cor_id) {
  uint64_t starttime = get_sys_clock();
  RC rc = RCOK;
...
RC YCSBTxnManager::redo_log(yield_func_t &yield, RC status, uint64_t cor_id) {
  ...
  for (int i = 0; i < ((YCSBQuery *)query)->requests.size(); i++) {
    ...
    if (status == RCOK) {  // validate success, log all replicas
      ...
      for (int j = 0; j < REPLICA_COUNT; j++) {
        ...
        if (loc == -1) {
          if (txn_stats.current_states == EXECUTION_PHASE)
            return Abort;
    ...
    for (int j = 0; j < node_id.size(); j++) {
      ...
      #if REPLICA_COUNT != 0
      bool is_primary = (node_id[j] == get_node_id_new(0,part_id));
      #else
      bool is_primary = (node_id[j] == get_primary_node_id(part_id));
      #endif
      ...
      change[node_id[j]].push_back(newChange);
    }
  }
#if RDMA_DBPAOR
  // batch faa
  ...
  for (int i = 0; i < g_node_cnt && rc == RCOK; i++) {
    if (change_cnt[i] > 0) {
      ...
      uint64_t result = -1;
      rc = faa_remote_content(yield, i, rdma_buffer_size - rdma_log_size + sizeof(uint64_t),
                              num_of_entry, &result, cor_id, count + 1, true);
  ...
  for (int i = 0; i < g_node_cnt && rc == RCOK; i++) {
    if (change_cnt[i] > 0) {
      if (i == g_node_id) {  // local log
        while (
            (start_idx[i] + num_of_entry - *(redo_log_buf.get_head()) >= redo_log_buf.get_size()) &&
            !simulation->is_done()) {
        }
        if (simulation->is_done()) break;
      } else {  // remote log
        pthread_mutex_lock(LOG_HEAD_LATCH[i]);
        while (start_idx[i] + num_of_entry - log_head[i] >= redo_log_buf.get_size() &&
               !simulation->is_done()) {
  ...
  for (int i = 0; i < g_node_cnt && rc == RCOK; i++) {
    if (change_cnt[i] > 0) {
      ...
      if (i == g_node_id) {  // local log
        ...
        rc = write_remote_log(yield, i, sizeof(LogEntry), start_offset, (char *)newEntry, cor_id,
                              count + 1, true);
        if (rc != RCOK) {
          rc = rc == NODE_FAILED ? Abort : rc;
          return rc;
  ...
  for (int i = 0; i < g_node_cnt && rc == RCOK; i++) {
    if (change_cnt[i] > 0) {
      ...
      if (i == g_node_id) {  // local log
        ...
        // for head: there is local read and write, and remote RDMA read, conflicts are ignored
        // here, which might be problematic
        //  while(start_idx + num_of_entry < *(redo_log_buf.get_head())){
        //  	//wait for head to reset
        //  	assert(false);
        //  }
        while ((start_idx + num_of_entry - *(redo_log_buf.get_head()) >= redo_log_buf.get_size()) &&
               !simulation->is_done()) {
          // wait for AsyncRedoThread to clean buffer
        }
        if (simulation->is_done()) break;
        ...
        while (start_idx + num_of_entry - log_head[i] >= redo_log_buf.get_size() &&
               !simulation->is_done()) {
          // wait for AsyncRedoThread to clean buffer
          rc = read_remote_log_head(yield, i, &log_head[i], cor_id);
          // todo: I do not know whether I should abort here.
  ...
  for (int i = 0; i < g_node_cnt && rc == RCOK; i++) {
    if (node_need_write_log[i] > 0) {
      ...
      if (i == g_node_id) {  // local log
        ...
        rc = write_remote_log(yield, i, sizeof(LogEntry), start_offset, (char *)le, cor_id,
                              count + 1, true);
        if (rc != RCOK) {
          rc = rc == NODE_FAILED ? Abort : rc;
          return rc;
...
void YCSBTxnManager::update_query_status(uint64_t return_id, OpStatus status) {
  YCSBQuery *ycsb_query = (YCSBQuery *)query;
  DEBUG_T("txn %ld update query status return id %ld, status %ld\n", get_txn_id(), return_id, status);
  for (int i = 0; i < ycsb_query->requests.size(); i++) {
    ycsb_request *req = ycsb_query->requests[i];
#if REPLICA_COUNT != 0
    for (int j = 0; j < req->replica_cnt; j++) {
      update_single_query_status(req->replica_node[j], return_id, status);
    }
#else
...
RC YCSBTxnManager::check_query_status(OpStatus status) {
  ...
  // printf("txn %ld check query status %ld\n", get_txn_id(), status);
  // 检查当前消息是否收集全了
  for (int i = 0; i < ycsb_query->requests.size(); i++) {
    ycsb_request *req = ycsb_query->requests[i];
    // printf("txn %ld req %ld status [1]%ld:%ld [2]%ld:%ld [3]%ld:%ld\n", get_txn_id(), i, req->replica_node[0].execute_node, req->replica_node[0].status, req->replica_node[1].execute_node, req->replica_node[1].status, req->replica_node[2].execute_node, req->replica_node[2].status);
    if (!enable_read_only_optimization && REPLICA_CC || req->acctype == WR) {
      uint64_t req_abort_cnt = 0;  // 当前操作的几个副本中，返回回滚的数量
      for (int j = 0; j < req->replica_cnt; j++) {
        if (req->replica_node[j].status == PREP_ABORT) req_abort_cnt++;
      }
#if REPLICA_CC
      if (req_abort_cnt > (REPLICA_COUNT * 1) / MINOR_REPLICA) {
        // printf("txn %ld need abort, due to %d secondary abort\n", get_txn_id(), i, req_abort_cnt);
        return Abort;
      }
#else
      if (req_abort_cnt > (REPLICA_COUNT * 1) / 2) {
        DEBUG_T("txn %ld need abort, due to req %d secondary status %ld, %ld abort\n", get_txn_id(),
                i, req->second1.status, req->second2.status);
        return Abort;
      }
#endif
      uint64_t req_no_complete_cnt = 0;  // 当前操作的几个副本中，没有收集的数量
      uint64_t no_com[REPLICA_COUNT] = {-1};
      uint64_t no_com_cnt = 0;
      for (int j = 0; j < req->replica_cnt; j++) {
        if (req->replica_node[j].status < status) {
          req_no_complete_cnt++;
          no_com[no_com_cnt++] = req->replica_node[j].execute_node;
        }
      }
#if REPLICA_CC
      if (req_no_complete_cnt > (REPLICA_COUNT * 1) / MINOR_REPLICA) {
        // printf("txn %ld need wait, due to req %d num %d secondary %d, %d, %d do not reach %ld\n", get_txn_id(), i, req_no_complete_cnt, no_com[0], no_com[1], no_com[2], status);
        return WAIT;
      }
#else
      if (req_no_complete_cnt > (REPLICA_COUNT * 1) / 2) {
        DEBUG_T("txn %ld need wait, due to req %d secondary status %ld,%ld:%ld\n", get_txn_id(), i,
                req->second1.status, req->second2.status, status);
        return WAIT;
      }
#endif
    } else {
      // 读操作
      if (req->replica_node[0].status == PREP_ABORT) {
        return Abort;
      } else if (req->replica_node[0].status < status) {
        // printf("txn %ld need wait, due to req %d num %d secondary %d do not reach %ld\n", get_txn_id(), i, req->replica_node[0].execute_node, status);
        return WAIT;
      }
    }
  }
  DEBUG_T("txn %ld check query status %ld success %d\n", get_txn_id(), status, can_enter_next_state);
  ...
  for (int i = 0; i < ycsb_query->requests.size(); i++) {
    ...
    if (req->primary.status == PREP_ABORT) {
      ...
      DEBUG_T("txn %ld need wait, due to req %d primary status %ld:%ld\n", get_txn_id(), i,
              req->primary.status, status);
      return WAIT;
    }
    if (req->acctype == WR) {
#if REPLICA_CC
      if (req->second1.status == PREP_ABORT || req->second2.status == PREP_ABORT) {
#else
      if (req->second1.status == PREP_ABORT && req->second2.status == PREP_ABORT) {
#endif
        DEBUG_T("txn %ld need abort, due to req %d secondary status %ld, %ld abort\n", get_txn_id(),
...
#if REPLICA_CC
      if (req->second1.status < status || req->second2.status < status) {
#else
      if (req->second1.status < status && req->second2.status < status) {
#endif
        DEBUG_T("txn %ld need wait, due to req %d secondary status %ld,%ld:%ld\n", get_txn_id(), i,
...
RC YCSBTxnManager::resend_remote_subtxn() {
  ...
  if (IS_LOCAL(get_txn_id()) || new_coordinator == g_node_id)
    query->partitions_touched.add_unique(GET_PART_ID(0, g_node_id));
  for (int i = 0; i < ycsb_query->requests.size(); i++) {
    ycsb_request *req = ycsb_query->requests[i];
    if (req->acctype != WR) {
    ...
    #if REPLICA_COUNT != 0
    for (int j = 0; j < REPLICA_COUNT; j ++) {
      if (req->replica_node[j].status == COMMIT_RESEND){
        loc = get_node_id_new(j, part_id);
        if (loc == -1) return WAIT;
        node_id.push_back(loc);
        req->replica_node[j].stored_node = loc;

        req->replica_node[j].status == PREPARE;
        DEBUG_T("resend-detail resend txn %ld req %ld replica node %ld to %ld\n", get_txn_id(), i, j, loc);
      }
    }
    #else
  ...
  for (auto iter = center_master.begin(); iter != center_master.end(); iter++) {
    ...
    for (int i = 0; i < ycsb_query->requests.size(); i++) {
      ...
      for (int i = 0; i < REPLICA_COUNT; i ++) {
        if (req->replica_node[i].status == COMMIT_RESEND &&
          GET_CENTER_ID(req->replica_node[i].stored_node) == center_id)
        req->replica_node[i].execute_node = executore_id;
      }
      #else
      if (req->primary.status == COMMIT_RESEND &&
          GET_CENTER_ID(req->primary.stored_node) == center_id)
        req->primary.execute_node = executore_id;
      if (req->second1.status == COMMIT_RESEND &&
          GET_CENTER_ID(req->second1.stored_node) == center_id)
        req->second1.execute_node = executore_id;
      if (req->second2.status == COMMIT_RESEND &&
          GET_CENTER_ID(req->second2.stored_node) == center_id)
        req->second2.execute_node = executore_id;
      #endif
    }
  }
