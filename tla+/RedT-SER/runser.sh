...
CONFIGS=(
    ...
    "KEY = {k1} CLIENT = {c1, c2, c3} REPLICA_COUNT = 1 QUORUM_SIZE = 1  MAX_TXN_PER_CLIENT = 1 MAX_OPS_PER_TXN = 1"
)
...
EOL
    ...
    # 优化JVM参数
    java -XX:+UseParallelGC -Xmx8192m -cp tla2tools.jar tlc2.TLC RedTSER.tla -config "$config_file" -workers auto 2>&1 | tee -a "$LOG_DIR/test_${test_number}_${timestamp}.log"
