...
/*!
  ...
  Suppose we want to init the heap identified as id 73, with buffer (ptr,4KB size); we do the
  following:
  ...
  use Alloc = AllocatorMaster<73>;
      ...
      char *ptr = (char *)(Alloc::get_thread_allocator()->alloc(4096); // similar to `char *ptr =
  malloc(4096);` Alloc::get_thread_allocator()->dealloc(ptr);                     // similar to
  `free(ptr);`
...
class AllocatorMaster {
 public:
  ...
  static Allocator *get_thread_allocator() {
    if (likely(thread_allocator)) return thread_allocator;
  ...
  static Allocator *get_allocator() {
    ...
    mallctl("arenas.create", (void *)(&arena_id), &sz, (void *)(&new_hooks),
  ...
  static u64 total_managed_mem() { return end_addr - start_addr; }
 ...
 private:
  ...
  static void *extent_alloc_hook(extent_hooks_t *extent_hooks, void *new_addr, size_t size,
    ...
    if ((uintptr_t)ret % alignment != 0) ret = ret + (alignment - (uintptr_t)ret % alignment);
...
template <int N>
char *AllocatorMaster<N>::start_addr = nullptr;
template <int N>
char *AllocatorMaster<N>::end_addr = nullptr;
template <int N>
char *AllocatorMaster<N>::heap_top = nullptr;
template <int N>
std::mutex AllocatorMaster<N>::lock;
...
extent_hooks_t AllocatorMaster<N>::hooks = {
    AllocatorMaster<N>::extent_alloc_hook,        AllocatorMaster<N>::extent_dalloc_hook,
    AllocatorMaster<N>::extent_destroy_hook,      AllocatorMaster<N>::extent_commit_hook,
    AllocatorMaster<N>::extent_decommit_hook,     AllocatorMaster<N>::extent_purge_lazy_hook,
    AllocatorMaster<N>::extent_purge_forced_hook, AllocatorMaster<N>::extent_split_hook,
