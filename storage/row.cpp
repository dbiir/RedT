...
#elif CC_ALG == RDMA_NO_WAIT || CC_ALG == RDMA_NO_WAIT3
...
RC row_t::get_row(yield_func_t &yield,access_t type, TxnManager *txn, Access *access,uint64_t cor_id) {
  ...
  INC_STATS(txn->get_thd_id(), trans_cur_row_copy_time, get_sys_clock() - copy_time);
	goto end;
#endif
#if CC_ALG == WAIT_DIE || CC_ALG == NO_WAIT || CC_ALG == RDMA_NO_WAIT || CC_ALG == WOUND_WAIT || CC_ALG == RDMA_NO_WAIT3
  	uint64_t init_time = get_sys_clock();
    ...
    INC_STATS(txn->get_thd_id(), trans_cur_row_init_time, get_sys_clock() - init_time);
	if (!txn->is_recover) {
		if (txn->enable_read_only_optimization) {
			rc = this->manager->read_only_get(txn->get_start_timestamp(),idx, txn, this);
		} else {
			rc = this->manager->lock_get(yield,lt,txn,this,cor_id);
		}
	}
...
RC row_t::get_row_post_wait(access_t type, TxnManager * txn, row_t *& row) {
	RC rc = RCOK;
  uint64_t init_time = get_sys_clock();
	assert(CC_ALG == WAIT_DIE || CC_ALG == MVCC || CC_ALG == TIMESTAMP || CC_ALG == TIMESTAMP || CC_ALG == WOUND_WAIT);
