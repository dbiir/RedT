...
#elif CC_ALG == TICTOC 
    // manager = (Row_tictoc *) mem_allocator.align_alloc(sizeof(Row_tictoc));
...
RC row_t::get_row_post_wait(access_t type, TxnManager * txn, row_t *& row) {
  ...
  RC rc = RCOK;
  assert(CC_ALG == WAIT_DIE || CC_ALG == MVCC || CC_ALG == WOOKONG || CC_ALG == TIMESTAMP || CC_ALG == TICTOC || CC_ALG == SSI || CC_ALG == WSI);
#if CC_ALG == WAIT_DIE
...
#elif CC_ALG == MVCC || CC_ALG == TIMESTAMP || CC_ALG == WOOKONG || CC_ALG == TICTOC || CC_ALG == SSI || CC_ALG == WSI
