...
class SimpleAllocator : public AbsRecvAllocator {
 ...
 public:
  SimpleAllocator(Arc<RMem> mem, mr_key_t key) : buf(mem->raw_ptr), total_mem(mem->sz), key(key) {
...
class Transport {
 public:
  ...
  uint64_t get_twoside_port_id(uint64_t src_node_id, uint64_t dest_node_id,
