...
public:
  ...
  uint64_t pid;
#if CC_ALG == WAIT_DIE || CC_ALG == RDMA_WAIT_DIE2 || CC_ALG == TIMESTAMP || CC_ALG == MVCC || CC_ALG == DTA || CC_ALG == WOOKONG || CC_ALG == RDMA_WOUND_WAIT || CC_ALG == CICADA || CC_ALG == WOUND_WAIT
  uint64_t ts;
#endif
