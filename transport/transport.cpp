...
void Transport::create_server(uint64_t port, uint64_t dest_node_id) {
  ...
  pthread_mutex_unlock(tport_man.latch);
  for (RecvIter<Dummy, RDMA_ENTRY_NUM> iter(recv_qpi, recv_rsi); iter.has_msgs(); iter.next()) {
...
void Transport::create_client(uint64_t port, uint64_t dest_node_id) {
  ...
  auto qp_res = cm.cc_rc_msg("rdma_qp" + to_string(port), RDMA_CQ_NAME + to_string(port), 4096, qp,
                             RDMA_USE_NIC_IDX, QPConfig());
  ...
  auto local_mr =
      RegHandler::create(Arc<RMem>(new RMem(RDMA_BUFFER_SIZE /*1024 * 1024*/)), tport_man.nic)
          .value();
  ...
  for (int i = 1; i <= 1; ++i) {
    ...
    auto res_s = send_qpi->send_qps->send_normal(
        {.op = IBV_WR_SEND_WITH_IMM,
         ...
         .len = (u32)msg.size() + 1,
         .wr_id = 0},
        {.local_addr = reinterpret_cast<RMem::raw_ptr_t>(buf2), .remote_addr = 0, .imm_data = 0});
  ...
  for (uint64_t node_id = 0; node_id < g_total_node_cnt; node_id++) {
    ...
    if (ISCLIENTN(node_id)) {
      ...
      for (uint64_t server_thread_id = g_thread_cnt + g_rem_thread_cnt;
        ...
        th_bind.push_back(std::thread([=]() {
          uint64_t port_id =
              get_twoside_port_id(node_id, g_node_id, server_thread_id % g_send_thread_cnt);
          create_server(port_id, node_id);
        }));
      }
    }
    ...
    if (ISCLIENTN(g_node_id)) {
      ...
      for (uint64_t server_thread_id = g_thread_cnt + g_rem_thread_cnt;
        ...
        th_connect.push_back(std::thread([=]() {
          ...
          // cout << g_node_id << node_id << "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaa" << endl;
          uint64_t port_id =
              get_twoside_port_id(g_node_id, node_id, server_thread_id % g_send_thread_cnt);
          create_client(port_id, node_id);
        }));
      }
    }
...
void Transport::init() {
  ...
  for (uint64_t node_id = 0; node_id < g_total_node_cnt; node_id++) {
    ...
    if (ISCLIENTN(node_id)) {
      ...
      for (uint64_t server_thread_id = 0; server_thread_id < g_client_thread_cnt;
           server_thread_id++) {
        th_bind.push_back(std::thread([=]() {
    ...
    // Sending ports
    if (ISCLIENTN(g_node_id)) {
      for (uint64_t client_thread_id = 0; client_thread_id < g_client_thread_cnt;
           client_thread_id++) {
        th_connect.push_back(std::thread([=]() {
          sleep(1);
          // cout << g_node_id << node_id << "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaa" << endl;
          uint64_t port_id = get_thd_port_id(g_node_id, node_id, client_thread_id);
          create_client(port_id, node_id);
        }));
      }
    } else {
...
void Transport::rdma_thd_send_msg(uint64_t send_thread_id, uint64_t dest_node_id, Message *msg) {
  ...
  DEBUG("%ld Buffered Msg %d, (%ld,%ld) to %ld\n", send_thread_id, msg->rtype, msg->txn_id,
        msg->batch_id, dest_node_id);
  sbuf->cnt += 1;
...
void Transport::rdma_send_msg(uint64_t send_thread_id, uint64_t dest_node_id, char *sbuf,
                              int size) {
  uint64_t starttime = get_sys_clock();
...
#if USE_RDMA == CHANGE_TCP_ONLY
  if (ISCLIENTN(g_node_id)) {
    port_id = get_twoside_port_id(
        g_node_id, dest_node_id,
        (g_client_thread_cnt + g_client_rem_thread_cnt) % g_client_send_thread_cnt);
  } else {
    port_id = get_twoside_port_id(g_node_id, dest_node_id,
  ...
  auto res_s = send_qpi->send_qps->send_normal(
      {.op = IBV_WR_SEND_WITH_IMM,
       ...
       .wr_id = 0},
      {.local_addr = reinterpret_cast<RMem::raw_ptr_t>(buf), .remote_addr = 0, .imm_data = 0});
  RDMA_ASSERT(res_s == IOCode::Ok);
  // void* pVoid = buf;
  // RDMA_LOG(4) << port_id-TPORT_TWOSIDE_PORT << " RDMA send " << (send_qpi->count) << " buf size
  // "<< (u32) size + sizeof(size) + 1 << " buf ptr " << pVoid << " flags " << send_flags;
  if (send_qpi->count >= send_qpi->max_count) {
...
std::vector<Message *> *Transport::rdma_recv_msg(uint64_t thd_id) {
  ...
  if (ISCLIENTN(g_node_id)) {
    port_id = get_twoside_port_id(
        ctr, g_node_id, (g_client_thread_cnt + g_client_rem_thread_cnt) % g_client_send_thread_cnt);
  } else {
    port_id =
        get_twoside_port_id(ctr, g_node_id, (g_thread_cnt + g_rem_thread_cnt) % g_send_thread_cnt);
  }
#elif USE_RDMA == CHANGE_MSG_QUEUE
  ...
  while (bytes <= 0 && (!simulation->is_setup_done() ||
    ...
    if (ISCLIENTN(g_node_id)) {
      ...
      port_id = get_twoside_port_id(ctr, g_node_id,
                                    (g_thread_cnt + g_rem_thread_cnt) % g_send_thread_cnt);
    }
#elif USE_RDMA == CHANGE_MSG_QUEUE
...
char **Transport::get_ip() { return ifaddr; }
/*
